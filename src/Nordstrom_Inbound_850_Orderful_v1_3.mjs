import './styles.css';
import options from './850Nordstrom.json';

document.getElementById("app850").innerHTML = '850 Nordstom Script';
...
for (let i = 0; i < options.data.length; i++) {
    ...
    for (let j = 0; j < options.data[i].message.content.transactionSets.length; j++) {
      let record = options.data[i].message.content.transactionSets[j];
